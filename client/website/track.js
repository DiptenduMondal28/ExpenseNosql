...
async function showDelete(detail){
    ...
    deleteKey.id="deletekey";
    deleteKey.onclick=()=>{
        const token = localStorage.getItem('token')
        let element = axios.delete(`http://localhost:3000/deletadata/${detail._id}`,{headers:{'Authorization':token}});
...
async function premiumfeature(result){
    ...
    leaderboardButton.onclick=async()=>{
        ...
        userleaderboardArray.data.forEach(element => {
            leadderboardElement.innerHTML +=`<li>name:${element.userDetails[0].name} expence:${element.total}</li>`
        });
    }
